    ...
    var displayName = 'Gallery Description',
        schema = {
            properties: {
                ...
                isSessionEventWedding: {
                    description: 'Was this a [s]ession, [e]vent, or [w]edding?',
                    type: 'string',
                    pattern: /^(?:s|session|e|event|w|wedding)$/i,
                    message: 'Must be [s]ession, [e]vent, or [w]edding input',
                    required: true
                }
            }
        };
...
GalleryDescriptionMessageGenerator.prototype._generateMessageText = function (err, result, cb) {
    ...
    var eventType = result.isSessionEventWedding.toLowerCase();
    switch (eventType) {
        case 's':
            eventType = 'session';
            break;
    ...
    message += 'Thank you for having us capture your ' + eventType + '; we greatly appreciate your business and look forward to working with you again in the future.\n\n';
    message += 'This gallery contains all of the photos from your ' + eventType + '. ';
    message += 'By using the “Buy Photos” button to the right you can order individual prints and merchandise, as well as order a digital download of all the photos you see here. ';
    message += 'We highly recommend doing this on a computer, not a mobile or tablet device. Please note, watermarks do *not* show up on prints, merchandise, or downloads.\n\n';
